...
class LayoutDetectionYOLO:
    def __init__(self, config):
        ...
        # Default to GPU when available — historical 'cpu' default
        # silently bypassed GPU execution when YAML omitted `device:`.
    ...
    def predict(self, images, result_path, image_ids=None):
        ...
        """
        # Single batched ultralytics call so the GPU sees N pages at once
        # rather than N separate forward passes (~5x faster on V100 batches).
        raw_results = list(self.model.predict(
            images,
            imgsz=self.img_size,
