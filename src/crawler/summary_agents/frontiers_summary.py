...
DOMAIN_JOURNAL_MAP: Dict[str, str] = {
    "neuroscience": "Frontiers in Neuroscience",
    "psychology": "Frontiers in Psychology",
    "bio": "Frontiers in Biology",
    "medicine": "Frontiers in Medicine",
}


class FrontiersSummaryAgent(BaseSummaryAgent):
    ...
    async def fetch(self, intent: SearchIntent) -> List[PaperContent]:
        if not intent.keywords and not intent.domain:
            logger.warning("FrontiersSummaryAgent: requires keywords or domain")
            return []
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            while offset < intent.max_papers:
                ...
                try:
                    ...
                    logger.warning("Frontiers S2 query failed: %s", e)
                    break
    ...
    def _resolve_venue(self, intent: SearchIntent) -> str:
        if intent.venue:
            return intent.venue
        if intent.domain and intent.domain in DOMAIN_JOURNAL_MAP:
            return DOMAIN_JOURNAL_MAP[intent.domain]
        return ""
    ...
    def _parse_item(self, item: dict) -> Optional[PaperContent]:
        try:
            ...
            return PaperContent(
                paper_id=f"frontiers:{doi or paper_id}",
            ...
            logger.warning("Failed to parse Frontiers item: %s", e)
            return None

    def supported_domains(self) -> List[str]:
        return ["neuroscience", "psychology", "bio", "medicine"]
