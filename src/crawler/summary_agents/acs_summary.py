...
class ACSSummaryAgent(BaseSummaryAgent):
    ...
    async def fetch(self, intent: SearchIntent) -> List[PaperContent]:
        if not intent.keywords and not intent.domain:
            logger.warning("ACSSummaryAgent: requires keywords or domain")
            return []
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            while offset < intent.max_papers:
                ...
                try:
                    ...
                    logger.warning("ACS (S2 proxy) query failed: %s", e)
                    break
    ...
    def _build_query(self, intent: SearchIntent) -> str:
        ...
        return f'{base} "American Chemical Society"'
    ...
    def _parse_item(self, item: dict) -> PaperContent:
        try:
            ...
            logger.warning("Failed to parse ACS (S2) item: %s", e)
            return None
