...
class ACLSummaryAgent(BaseSummaryAgent):
    ...
    async def fetch(self, intent: SearchIntent) -> List[PaperContent]:
        if not intent.keywords and not intent.domain:
            logger.warning("ACLSummaryAgent: requires keywords or domain")
            return []
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            while offset < intent.max_papers:
                ...
                try:
                    ...
                    logger.warning("ACL S2 query failed: %s", e)
                    break
    ...
    def _parse_item(self, item: dict) -> Optional[PaperContent]:
        try:
            ...
            return PaperContent(
                paper_id=f"acl:{doi or paper_id}",
            ...
            logger.warning("Failed to parse ACL item: %s", e)
            return None
