...
class FrontiersPDFAgent(BasePDFAgent):
    ...
    async def download(
        ...
        if not pdf_url:
            return PDFDownloadResult(
                ...
                success=False,
                error="no_frontiers_pdf_url",
            )

        filename = f"{PDFDownloader.sanitize_filename(paper.title, 80)}.pdf"
        output_path = output_dir / "frontiers" / filename
    ...
    def can_download(self, paper: PaperContent) -> bool:
        if paper.pdf_url and "frontiersin.org" in paper.pdf_url:
            return True
        if paper.source == "frontiers":
            return True
        return False
    ...
    def _get_pdf_url(self, paper: PaperContent) -> Optional[str]:
        ...
        if paper.doi:
            return f"https://www.frontiersin.org/articles/{paper.doi}/pdf"
        return None
