...
class PLOSPDFAgent(BasePDFAgent):
    ...
    async def download(
        ...
        output_path = output_dir / "plos" / filename
    ...
    def _get_pdf_url(self, paper: PaperContent) -> Optional[str]:
        if paper.pdf_url and "plos.org" in paper.pdf_url:
            return paper.pdf_url
        if paper.source == "plos" and paper.doi:
            journal = paper.extra.get("journal", "plosone")
            return (
                f"https://journals.plos.org/{journal}/article/file"
                f"?id={paper.doi}&type=printable"
            )
        return None
